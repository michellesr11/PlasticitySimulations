...
% calculates plasticity index for pairs of postsynaptic potentials at
% variable inter-stimulus intervals. 
ISIs = [0.2]; % el intervalo entre estimulos (en segundos)
...
for i = 1:numel(ISIs)
    stim_times = [start_time start_time+ISIs(i)]; % stimulus times
    [t_steps,f_steps] = simulatePlast(2,CI,params,stim_times,step_size,maxtime); % simulacion
    % get the amplitude of the PSP as n*p
    amps = PSPamps(t_steps,f_steps,stim_times);
    % normalise them
    amps_norm = amps / amps(1);
...
params = [0 0.18 1 1 1.2 35 35 0.28 5.5 0.67]; % [b1 tau_f k1r k2r tau_b m1 m2 a_f y del]
b20 = 0.1;
% get steady state
...
if model == 1 % classic
    for int_period = 1:numel(tj)+1
        if int_period == 1
            ...
            if int_period > numel(tj)
                t_steps_temp = [tj(int_period-1):h:int_time]; % last integration period (no further events)
            else
...
elseif model == 3   % retrieval
    tj_orig = tj; % store events that correspond to stimulation
    ret = params(end);
    ...
    for int_period = 1:numel(tj)+1
        if int_period == 1 % baseline before any stimulation
            ...
            t_steps = t_steps_period;
            f_steps = f_steps_period; % valor de las varibales
        elseif int_period > 1 % each of the integration periods (periods between events)
            if int_period > numel(tj)
                t_steps_temp = [tj(int_period-1):h:int_time]; % last integration period (no further events)
            else
                t_steps_temp = [tj(int_period-1):h:tj(int_period)];
            ...
            % define state after release
            if ~isempty(find((tj(int_period-1) == tj_orig+ret))) % if event is delay
                newCI = [b2_period+params(9)*(p_max) f_steps_period(end,2) nT_period p_period];
            else % if event stimulus arrival
